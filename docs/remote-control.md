...
- **Use your full local environment remotely**: your filesystem, [MCP servers](https://docs.claude.com/en/mcp), tools, and project configuration all stay available, and typing `@` autocompletes file paths from your local project
...
Unlike [Claude Code on the web](https://docs.claude.com/en/claude-code-on-the-web), which runs on cloud infrastructure, Remote Control sessions run directly on your machine and interact with your local filesystem. The web and mobile interfaces are just a window into that local session.
...
In the [Claude Code VS Code extension](https://docs.claude.com/en/vs-code), type `/remote-control` or `/rc` in the prompt box, or open the command menu with `/` and select it. Requires Claude Code v2.1.79 or later.
...
- **Ultraplan disconnects Remote Control**: starting an [ultraplan](https://docs.claude.com/en/ultraplan) session disconnects any active Remote Control session because both features occupy the claude.ai/code interface and only one can be connected at a time.
...
- **The error mentions `disableRemoteControl`**: your IT administrator has disabled Remote Control on this device through [managed settings](https://docs.claude.com/en/settings#settings-files), independent of the organization-wide toggle.
...
| :--- | :--- | :--- | :--- | :--- |
| [Dispatch](https://docs.claude.com/en/desktop#sessions-from-dispatch) | Message a task from the Claude mobile app | Your machine (Desktop) | [Pair the mobile app with Desktop](https://support.claude.com/en/articles/13947068) | Delegating work while you're away, minimal setup |
| [Remote Control](https://docs.claude.com/en/remote-control) | Drive a running session from [claude.ai/code](https://claude.ai/code) or the Claude mobile app | Your machine (CLI or VS Code) | Run `claude remote-control` | Steering in-progress work from another device |
| [Channels](https://docs.claude.com/en/channels) | Push events from a chat app like Telegram or Discord, or your own server | Your machine (CLI) | [Install a channel plugin](https://docs.claude.com/en/channels#quickstart) or [build your own](https://docs.claude.com/en/channels-reference) | Reacting to external events like CI failures or chat messages |
| [Slack](https://docs.claude.com/en/slack) | Mention `@Claude` in a team channel | Anthropic cloud | [Install the Slack app](https://docs.claude.com/en/slack#setting-up-claude-code-in-slack) with [Claude Code on the web](https://docs.claude.com/en/claude-code-on-the-web) enabled | PRs and reviews from team chat |
| [Scheduled tasks](https://docs.claude.com/en/scheduled-tasks) | Set a schedule | [CLI](https://docs.claude.com/en/scheduled-tasks), [Desktop](https://docs.claude.com/en/desktop-scheduled-tasks), or [cloud](https://docs.claude.com/en/routines) | Pick a frequency | Recurring automation like daily reviews |

## Related resources

- [Claude Code on the web](https://docs.claude.com/en/claude-code-on-the-web): run sessions in Anthropic-managed cloud environments instead of on your machine
- [Ultraplan](https://docs.claude.com/en/ultraplan): launch a cloud planning session from your terminal and review the plan in your browser
- [Channels](https://docs.claude.com/en/channels): forward Telegram, Discord, or iMessage into a session so Claude reacts to messages while you're away
- [Dispatch](https://docs.claude.com/en/desktop#sessions-from-dispatch): message a task from your phone and it can spawn a Desktop session to handle it
- [Authentication](https://docs.claude.com/en/authentication): set up `/login` and manage credentials for claude.ai
- [CLI reference](https://docs.claude.com/en/cli-reference): full list of flags and commands including `claude remote-control`
- [Security](https://docs.claude.com/en/security): how Remote Control sessions fit into the Claude Code security model
- [Data usage](https://docs.claude.com/en/data-usage): what data flows through the Anthropic API during local and remote sessions
